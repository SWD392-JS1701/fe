...
import { getAllBookingsController, deleteBookingController } from "@/app/controller/bookingController";
...
import { setBookingCount, decrementBookingCount } from "@/lib/redux/bookingSlice";
...
const MyBookingsPage = () => {
        ...
        const bookingsWithDoctors = await Promise.all(
          userBookings.map(async (booking) => {
            try {
              ...
              console.error(`Error fetching doctor for booking ${booking._id}:`, error);
  ...
  const activeBookings = bookings.filter(booking => !isPast(new Date(booking.booking_time)));
  const pastBookings = bookings.filter(booking => isPast(new Date(booking.booking_time)));
            ...
            {activeBookings.map((booking) => (
              ...
              >
                <div className="flex items-center mb-4">
                  ...
                  />
                  <div className="ml-4">
                    <h3 className="font-semibold text-lg">{booking.doctor?.name || "Unknown Doctor"}</h3>
                    <p className="text-gray-600 text-sm">{booking.doctor?.specialties?.join(", ") || "Specialist"}</p>
                  </div>
                </div>
